...
class LevenbergMarquad {
 public:
  LevenbergMarquad(
    ...
    auto W = torch::diag(((1 / pow(sigma, 2)) * length(x)));
  }
  ...
  void broyden_jacobian_update(torch::Tensor J, torch::Tensor dp) {
    ...
    J += torch::outer(
        df - torch::mv(J, dp), dp)::div(torch::linalg::norm(dp, ord = 2));
  }
  ...
  void solve_for_dp() {
    ...
    torch::NoGradGuard no_grad;
    static torch::Tensor JTW =
        torch::matmul(torch::transpose(J, 0, 1), this->W);
    static torch::Tensor JTWJ = torch::matmul(JTW, J);

    torch::Tensor dy = y - func(p);
    torch::Tensor dp = torch::linalg::solve(
        JTWJ + lambda_lm * torch::diag(torch::diagonal(JTWJ)),
        torch::mv(JTW, dy));
  }
  ...
  void chi_2(torch::Tensor p, torch::Tensor& W, torch::Tensor& y) {
    ...
    return (
        torch::dot(y, torch::mv(W, y)) -
        2 * torch::dot(y, torch::mv(W, y_hat)) +
  ...
  bool rho(
    ...
    //    rho =  chi2(p) - chi2(p + dp) / (dp)^T . ( lambda * diag(J^T W J).dp +
    //    J^T W . dy )
